...
- `create_grooming_request`: creates the request and authorized matches from customer-owned pet data, fixed service type, validated location mode/address/state/ZIP/range, compatible groomer `service_location_modes` membership, active fixed groomer services, request-day groomer capacity, and backend-owned location plus pet-fit evidence scoring with capped low-confidence claim/portfolio signals that are suppressed when negative earned evidence exists. Offer creation and offer acceptance continue to enforce exact proposed-time availability.
...
- A customer cannot read/update another customer's account contact/address fields or avatar objects in `customer-avatars`.
