...
- [x] Add tests that require customer and groomer foreground tokens to meet 4.5:1 against their own gradient endpoints.
- [x] Add a test that requires the hero title to equal `Need grooming for your pet?` and contain no newline.
- [x] Run the focused tests and confirm RED because the role tokens and presentation copy do not exist.
...
- [x] Add deep customer-mint and deep groomer-coral foreground tokens.
- [x] Select the foreground through the primary button accent instead of a single black token.
- [x] Run the focused contrast tests and confirm GREEN.
...
- [x] Move hero strings into `CustomerHomeRequestHeroPresentation` and remove the embedded newline.
- [x] Match the reference's sampled light mint gradient, 110pt edge-aligned circle, and diagonal paw decoration.
- [x] Replace the local filled primary action with a compact white surface action using shared `customerHeroText #333333` foreground and feature-level semantic typography, with unchanged disabled semantics.
- [x] Keep the action content-sized and make the copy follow its measured width, with available-width contraction on narrower devices.
- [x] Run focused presentation and accessibility tests and confirm GREEN.
...
- [x] Add `customerAccent`, `customerAccentSoft`, `customerAccentSubtle`, and `customerAccentStrong` Display P3 roles and change global `textPrimary` to `#333333`.
- [x] Alias existing Hero values and temporary `customerPrimary` compatibility names to the new roles.
- [x] Update shared Customer action, selection, form, feedback, image, layout, progress, and tint primitives to consume roles by semantic purpose.
- [x] Audit remaining Customer feature references and direct colors; preserve Groomer and status semantics.
- [x] Add contrast/token contract tests for primary text, Customer action fills, strong graphics, and unchanged Groomer/status values.
