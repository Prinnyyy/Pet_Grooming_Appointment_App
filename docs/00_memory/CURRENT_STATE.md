...
- Supabase check: `./scripts/supabase-check.sh` passed for T-015 on 2026-06-20.
...
- Fresh marketplace baseline plus real Supabase email/password authentication, atomic role onboarding, profile loading, authenticated role routing, customer pet management, groomer profile management, request/match backend, offer backend, customer request publishing/list/detail UI, and groomer matched request feed/detail/dismiss UI; offer UI, booking, chat, and review marketplace flows are not implemented.
...
- T-008 pet tables/private photo bucket, T-010 groomer profile/services/portfolio backend, T-012 request/match backend, and T-015 offer backend are deployed and backend-validated. T-009 implements the iOS Storage API upload/delete path for pet photos and its approved remote upload/delete smoke passed. T-011 implements the iOS groomer profile/services/portfolio path, but remote portfolio upload/delete smoke has not been run. T-013 implements customer request publishing/list/detail, but request cancellation is blocked until a backend cancel RPC exists. T-014 implements groomer matched request feed/detail/dismiss. Offer UI, booking/chat/review tables, RPCs, and Storage buckets remain unimplemented.
...
- T-016 — implement groomer offer creation UI against the deployed T-015 backend; do not start automatically.
