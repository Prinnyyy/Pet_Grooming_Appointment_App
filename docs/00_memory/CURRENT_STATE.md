...
- Test result: passed for T-014 on 2026-06-20 with 41 Swift Testing tests and 1 XCTest UI smoke test (`** TEST SUCCEEDED **`).
...
- Groomer Requests now loads active own `request_matches` plus readable open/has-offers `grooming_requests` through `GroomerRequestRepository`, displays frozen request/pet details, and dismisses visible/viewed matches through `dismiss_request_match`. Offer creation is not connected.
...
- T-015 — add the groomer offer backend in a separate Deep task with explicit Supabase MCP validation; do not start automatically.
