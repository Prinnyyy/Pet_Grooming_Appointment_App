...
- Test result: passed for T-019 on 2026-06-20 with 55 Swift Testing tests and 1 XCTest UI smoke test (`** TEST SUCCEEDED **`).
...
- Structure: feature-first App, Core models/configuration/infrastructure/repositories, DesignSystem, Auth bootstrap, Customer tabs, Customer pets, Customer requests, Bookings, Groomer tabs, Groomer requests, and Groomer profile management.
- Supabase Swift is pinned exactly to 2.46.0 with a checked-in package lock. App composition creates the client plus injectable Auth, profile, customer pet, customer request, booking, groomer profile, and groomer request repositories; SwiftUI views do not access Supabase directly.
...
- `RoleOnboardingView` requires a 1–80 character trimmed display name and explicit Customer/Groomer selection. Customer role receives customer pet management on Home, request publishing/list/detail/offer acceptance on Requests, booking list/detail/cancel on Bookings, and Account sign-out. Groomer role receives matched request feed/detail/dismiss/offer creation on Requests, booking list/detail/cancel on Bookings, and profile/services/portfolio management in Account plus an account/sign-out link; the separate Offers and Messages tabs remain placeholder-only.
...
- The iOS app performs real Supabase Auth, profile lookup/onboarding, customer pet CRUD, pet-photo metadata, groomer profile/service CRUD, groomer portfolio metadata, customer grooming request creation/owned-request reads/customer offer reads/offer acceptance, groomer matched request reads/dismissal, groomer offer creation/withdrawal, participant booking reads/cancellation, and private `pet-photos`/`groomer-portfolio` Storage upload/delete operations through repository boundaries.
...
- T-020 — implement booking-participant chat; do not start automatically.
