...
- T-019 booking acceptance and role UI is completed at `docs/06_tasks/T-019_BOOKING_ACCEPTANCE_ROLE_UI.md`; Customer request detail can accept an eligible offer through `accept_groomer_offer`, both roles have Bookings tab list/detail UI with short support references, and confirmed bookings can be cancelled through `cancel_booking`.
- T-020 booking participant chat is completed at `docs/06_tasks/T-020_BOOKING_PARTICIPANT_CHAT.md`; the migration is applied and mirrored, backend behavior checks passed, and both role Messages tabs support text-only participant conversations through repository boundaries.
...
- Structure: feature-first App, Core models/configuration/infrastructure/repositories, DesignSystem, Auth bootstrap, Customer tabs, Customer pets, Customer requests, Bookings, Chat, Groomer tabs, Groomer requests, and Groomer profile management.
- Supabase Swift is pinned exactly to 2.46.0 with a checked-in package lock. App composition creates the client plus injectable Auth, profile, customer pet, customer request, booking, chat, groomer profile, and groomer request repositories; SwiftUI views do not access Supabase directly.
...
- `RoleOnboardingView` requires a 1–80 character trimmed display name and explicit Customer/Groomer selection. Customer role receives customer pet management on Home, request publishing/list/detail/offer acceptance on Requests, booking list/detail/cancel on Bookings, participant text chat on Messages, and Account sign-out. Groomer role receives matched request feed/detail/dismiss/offer creation on Requests, booking list/detail/cancel on Bookings, participant text chat on Messages, and profile/services/portfolio management in Account plus an account/sign-out link; the separate Offers tab remains placeholder-only.
...
- Customer Requests now loads real customer-owned pets and grooming requests, validates request drafts, publishes through `create_grooming_request`, reloads authoritative owned requests, shows match-count feedback, displays frozen pet/request details, reviews received offers with pending offers separated from historical offers plus active groomer summaries through `CustomerRequestRepository`, and accepts eligible pending offers through `BookingRepository.acceptOffer`. Request cancellation is not connected because the backend has no customer request cancel RPC. T-019 acceptance refreshes authoritative request/offer state and explains booking conflicts without fabricating a local booking.
...
- Customer and Groomer Bookings tabs load participant-readable `bookings` through `BookingRepository`, show shared booking list/detail UI with short support reference codes, call `cancel_booking` only for confirmed bookings, and clearly state that booking cancellation does not reopen the original request or offers. Missing local booking/offer update targets now produce refresh hints instead of silent no-ops. Customer and Groomer Messages tabs load participant-readable `conversations` and `messages` through `ChatRepository`, show booking schedule/price context, display active groomer business names to customers when existing RLS permits it, support trimmed text-only sends, and keep realtime/attachments deferred. Completion and reviews remain T-021.
...
- T-008 pet tables/private photo bucket, T-010 groomer profile/services/portfolio backend, T-012 request/match backend, T-015 offer backend, T-018 booking/conversation backend, and T-020 text-message backend are deployed and backend-validated. T-009 implements the iOS Storage API upload/delete path for pet photos and its approved remote upload/delete smoke passed. T-011 implements the iOS groomer profile/services/portfolio path, but remote portfolio upload/delete smoke has not been run. T-013 implements customer request publishing/list/detail, but request cancellation is blocked until a backend request-cancel RPC exists. T-014 implements groomer matched request feed/detail/dismiss. T-016 implements groomer-side offer creation/withdrawal. T-017 implements customer offer review. T-019 implements offer acceptance plus customer/groomer booking list/detail/cancel UI while preserving T-018's no-reopen cancellation rule. T-020 implements text-only participant chat with booking context; groomer-side customer names still need a future customer-profile presentation contract, and realtime, chat attachments, completion, and reviews remain unimplemented.
...
- T-021 — implement booking completion and completed-only reviews; do not start automatically.
