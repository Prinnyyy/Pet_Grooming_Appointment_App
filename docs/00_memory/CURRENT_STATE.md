...
- Test result: passed for T-006 on 2026-06-20 with 10 Swift Testing tests and 1 XCTest UI smoke test (`** TEST SUCCEEDED **`).
...
- Fresh marketplace baseline plus real Supabase email/password authentication; no role onboarding or product flow is implemented.
- Production launch validates configuration, restores/observes the real Auth session, and displays Sign In/Create Account or onboarding-required. Missing configuration remains blocking, and email-confirmation sign-up never fabricates a session.
...
- Every authenticated session stops at onboarding-required; no profile query/write, role selection, Customer/Groomer routing, product query, runtime demo mode, or token exposure exists.
...
- The iOS app now performs real Supabase Auth operations but does not query or mutate the deployed profile schema.
...
- Only the T-004 profile/avatar backend foundation is deployed. All product-domain tables, RPCs, and later Storage buckets remain unimplemented.
...
- T-007 — implement role onboarding, profile creation, and authenticated role routing. Do not start it automatically.
