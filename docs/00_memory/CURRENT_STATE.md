...
- Date: 2026-07-09
- Updated by: Codex
- Latest completed task: T-215 Republish hardening.
- Current task: none; T-157 APNs remains externally blocked.
- Next task ID: use T-216 unless the user resumes T-157 after Apple Developer Program upgrade.
...
- Last iOS build/test: `./scripts/ios-build.sh` and `./scripts/ios-test.sh` passed on 2026-07-09 during T-215 republish hardening.
- Last focused iOS validation: T-215 CustomerRequestsStore tests covered unavailable original requests, expired preferred windows, and missing request photos in republish.
...
- Recent focused validations: T-206 offer, T-208 notification, T-209 reminder, T-210 chat/badge, T-214 decode/cache, and T-215 republish tests passed.
- Last docs validation: T-215 context hygiene and `git diff --check` passed on 2026-07-09.
...
- V1.0 ideal-operation packages are adopted in `docs/06_tasks/ROADMAP_EXECUTION_QUEUE.md`; Q-16...Q-27 are complete, and sequencing continues at Q-28.
...
- Use T-216 next. Recommended package is Q-28 state-machine edge tests. T-203 migration remains local-only until explicit remote apply authorization. Q-90...Q-92 remain blocked on Apple/APNs/release or production SMTP credentials.
