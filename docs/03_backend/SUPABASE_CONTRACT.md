...
`profiles`, `customer_profiles`, base `groomer_profiles`, `pets`, `pet_photos`, T-010 groomer profile details, `groomer_services`, `groomer_portfolio_photos`, `groomer_availability_windows`, `groomer_booking_preferences`, `groomer_time_off_windows`, `groomer_fit_claims`, `groomer_portfolio_fit_tags`, `grooming_requests`, `request_matches`, `groomer_offers`, `bookings`, `conversations`, `messages`, `reviews`, and `review_pet_fit_outcomes` are deployed and backend-validated. T-065 private pet-fit SQL helper functions, T-068 `groomer_pet_fit_evidence_summary`, and T-069 pet-fit match scoring inside `create_grooming_request` are deployed and backend-validated. T-070 is iOS-only and deploys no schema. Later availability enforcement remains planned only until its owning task applies and verifies a migration.
