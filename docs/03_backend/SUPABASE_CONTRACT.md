...
This backend contract is derived from `Fresh_Pet_Groomer_Marketplace_Engineering_Brief.md`. The T-004 profile/avatar foundation, T-007 atomic profile-onboarding RPC, T-008 pet/photo schema and private bucket, T-010 groomer profile/services/portfolio backend, T-012 grooming request/match backend, T-015 groomer offer backend, T-018 booking/conversation backend, T-020 text-message backend, T-021 completion/review backend, T-044 customer request cancellation backend, T-049 fixed-service/location/request-photo contract, T-058 groomer availability windows, and T-059 groomer full-address/multi-location profile contract are deployed to the fresh Supabase project and mirrored under `supabase/migrations/`. T-059 migration metadata, grants, function shape, trigger shape, and advisors were verified under the approved MCP-only boundary. The original project visible through MCP is a legacy project and is not a target for this rebuild.
...
`profiles`, `customer_profiles`, base `groomer_profiles`, `pets`, `pet_photos`, T-010 groomer profile details, `groomer_services`, `groomer_portfolio_photos`, `groomer_availability_windows`, `grooming_requests`, `request_matches`, `groomer_offers`, `bookings`, `conversations`, `messages`, and `reviews` are deployed and backend-validated. Every other row remains planned until its owning task applies and verifies a migration.
...
- Groomer availability is stored as one weekly window per ISO weekday for the owning groomer. T-058 does not connect availability to request matching or booking conflict checks; that remains a future backend/model task.
