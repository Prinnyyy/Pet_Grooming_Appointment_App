...
This backend contract is derived from `Fresh_Pet_Groomer_Marketplace_Engineering_Brief.md`. The T-004 profile/avatar foundation, T-007 atomic profile-onboarding RPC, T-008 pet/photo schema and private bucket, T-010 groomer profile/services/portfolio backend, T-012 grooming request/match backend, T-015 groomer offer backend, T-018 booking/conversation backend, T-020 text-message backend, T-021 completion/review backend, T-044 customer request cancellation backend, T-049 fixed-service/location/request-photo contract, and T-058 groomer availability windows are deployed to the fresh Supabase project and mirrored under `supabase/migrations/`. T-058 migration metadata, grants, RLS policies, and trigger shape were verified under the approved MCP-only boundary. The original project visible through MCP is a legacy project and is not a target for this rebuild.
...
- Active groomer profiles must include base city/state/radius and `service_location_mode`; request matching requires compatible service-location mode and at least one active groomer service with the requested fixed service type.
