...
- Remote verification date: 2026-06-25.
...
`profiles`, `customer_profiles`, base `groomer_profiles`, `pets`, `pet_photos`, T-010 groomer profile details, `groomer_services`, `groomer_portfolio_photos`, `groomer_availability_windows`, `groomer_booking_preferences`, `groomer_time_off_windows`, `groomer_fit_claims`, `groomer_portfolio_fit_tags`, `grooming_requests`, `request_matches`, `groomer_offers`, `bookings`, `conversations`, `messages`, `reviews`, and `review_pet_fit_outcomes` are deployed and backend-validated. T-065 private pet-fit SQL helper functions, T-068 `groomer_pet_fit_evidence_summary`, T-069 pet-fit match scoring inside `create_grooming_request`, T-071 availability enforcement inside offer/acceptance RPCs, T-072 availability-aware request matching, T-073 low-confidence claim/portfolio match signals, and T-074 customer offer match evidence RLS are deployed and backend-validated. T-070 is iOS-only and deploys no schema.
...
|---|---|---|---|
| `groomer_pet_fit_evidence_summary` | Derived summary of completed similar bookings, positive/negative structured outcomes, evidence timestamps, and confidence tier by groomer/trait | Read-only `security_invoker`/`security_barrier` view; authenticated and service_role have SELECT only; no anon grant; direct client reads obey underlying table RLS, while controlled backend contexts can consume full aggregate evidence | T-068 |
...
- Structured review outcomes are created only through `create_review`, use canonical T-065 trait type/value pairs, and store `positive` or `negative` outcomes. They do not change request distribution until a later matching task consumes them.
...
- `request_matches.match_score` and `request_matches.match_reason` remain backend-generated, bounded, and explainable. T-069 combines location fit with completed-booking and structured-review evidence from `groomer_pet_fit_evidence_summary`; T-073 adds a separate low-confidence claim/portfolio signal cap after hard eligibility and availability filters pass. The client may display results but does not calculate authoritative matching. Customers can read these fields only for matches already linked to offers on their owned requests.
