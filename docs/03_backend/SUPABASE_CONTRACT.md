...
- `request_matches.match_score` and `request_matches.match_reason` remain backend-generated, bounded, and explainable. T-069 combines location fit with completed-booking and structured-review evidence from `groomer_pet_fit_evidence_summary`; the client may display results but does not calculate authoritative matching.
