...
| `grooming_requests` | Published customer request with frozen pet/photo snapshot | `id`, customer/pet references, snapshots, service/time/location, status/expiry, timestamps | Customer reads own and cancels open/offer states through `cancel_grooming_request`; matched groomer reads only open/offer-eligible assigned requests; direct client writes denied | T-012, T-044 |
