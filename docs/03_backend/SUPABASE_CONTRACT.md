...
| `groomer_availability_windows` | Groomer's weekly availability editor state | `id`, `groomer_id`, ISO weekday, start/end time, enabled flag, timezone, timestamps | Groomer reads and replaces own weekly rows; T-071 uses enabled rows inside controlled offer creation and offer acceptance checks | T-058, T-071 |
| `groomer_booking_preferences` | Groomer's availability booking preferences | `groomer_id`, max appointments per day, minimum advance notice days, auto-accept flag, timestamps | Groomer reads and upserts own row; T-071 enforces max daily appointments and minimum advance notice in offer creation and acceptance; auto-accept remains unused | T-060, T-071 |
| `groomer_time_off_windows` | Groomer's unavailable date windows | `id`, `groomer_id`, title, start/end date, timestamps | Groomer reads/creates/updates/deletes own rows; T-071 blocks offer creation and acceptance for local dates inside time off | T-060, T-071 |
