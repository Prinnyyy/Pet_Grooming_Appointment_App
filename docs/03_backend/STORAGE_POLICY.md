...
| Read | Owner plus explicitly authorized profile presentation | Owning customer; request flow uses frozen authorized snapshot/metadata | Owner plus authenticated object reads for active groomer portfolio metadata; broad listing denied | Owning customer and matched groomers through `request_photos`/request-match policies | Conversation participants only |
