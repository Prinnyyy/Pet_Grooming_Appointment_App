...
| Groomer profile/services/portfolio/availability | read marketplace-safe active profile/service/portfolio data only | manage own profile, services, portfolio, weekly availability rows, booking preferences, time off rows, full base address fields, and profile avatar path/object | Verification/rating summary denied to client; availability preferences/time off are enforced by controlled offer and acceptance RPCs, not customer-readable slot discovery; broader customer-facing avatar presentation remains deferred |
