...
| Groomer profile/services/portfolio/availability | read marketplace-safe active profile/service/portfolio data only | manage own profile, services, portfolio, and availability rows | Verification/rating summary denied to client; availability is not customer-readable yet |
...
- `create_grooming_request`: creates the request and authorized matches from customer-owned pet data, fixed service type, validated location mode/address/state/ZIP/range, compatible groomer service-location mode, and active fixed groomer services.
