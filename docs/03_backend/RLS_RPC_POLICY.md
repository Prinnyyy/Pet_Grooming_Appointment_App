...
- `create_grooming_request`: creates the request and authorized matches from customer-owned pet data, fixed service type, validated location mode/address/state/ZIP/range, compatible groomer `service_location_modes` membership, active fixed groomer services, groomer availability for the preferred range, and backend-owned location plus pet-fit evidence scoring with capped low-confidence claim/portfolio signals that are suppressed when negative earned evidence exists.
