...
## Beckon UI Pass Rules

- Start Beckon UI work by extending shared `DesignSystem` tokens and primitives before restyling feature screens.
...
- If the Beckon prototype shows a state that current backend contracts do not support, document it as deferred instead of adding local fake success paths.
