...
T-301 adopts R-041 through `../superpowers/specs/2026-07-12-ui-consistency-governance-design.md`; T-303 defines Q-113 through Q-120 in `../superpowers/plans/2026-07-12-ui-consistency-governance-implementation.md`. Beckon's existing DesignSystem remains the single implementation source. The direction adds business-semantic typography/layout roles, reuse-backed presentation components, a tested source-audit debt ratchet, and explicit exceptions.

The first migration slice is Customer Home, Requests, Request creation, and Account plus their directly used DesignSystem primitives. All Feature code enters the audit baseline immediately, but non-slice legacy debt is recorded rather than forcing a whole-app rewrite. This work preserves business logic, repositories, navigation, backend contracts, dependencies, and the separately deferred Groomer Q-104 scope. Execution packages remain pending the post-spec implementation plan.
...
| M13 Address integrity and geospatial matching | Confirm Apple Maps service addresses and make PostGIS distance/radii the location authority. | Complete T-291/T-292/T-293/T-294/T-296/T-297/T-298/T-299/T-300; Q-105 through Q-112 complete. | One shared editor, coordinate-backed active profiles/requests, private geo data, both radius directions verified, and no city-text fallback remain. |
| M14 UI consistency governance | Make visual consistency code-defined, reusable, and automatically auditable. | Design T-301 and implementation plan T-303 complete; Q-113 through Q-120 queued. | Four Customer reference surfaces use semantic foundations/components, all Feature code blocks new visual debt, and remaining debt is explicit. |
...
| R-040 | M13 | Apple Maps address confirmation and PostGIS distance matching | Complete T-291 through T-300; Q-105 through Q-112 complete in `APPLE_MAPS_ADDRESS_SYSTEM_PLAN.md` |
| R-041 | M14 | Code-enforced UI consistency and Customer reference migration | Design T-301 and plan T-303 complete; execute Q-113 through Q-120 in order |
...
Execution sequencing: start R-041 with Q-113, then execute Q-114 through Q-120 in order. Q-104 remains deferred until the user restores its Groomer Dynamic Type/Accessibility scope; Q-90/Q-91 remain excluded, Q-92 is complete, and Q-93 remains externally blocked.
