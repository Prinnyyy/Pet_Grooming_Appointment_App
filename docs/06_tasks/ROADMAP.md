...
Q-97 through Q-104 are local iOS packages. They preserve existing repositories, marketplace state transitions, authenticated images, feedback, pagination, accessibility, and TestOps behavior. No Supabase migration, remote write, dependency, or deferred product feature is included.
...
| M12 Groomer workspace | Make Groomer operationally clear while preserving the shared Beckon product language. | Design T-249 complete; Q-97 through Q-104 queued. | Five direct tabs, approved screen hierarchy, focused editors, selector-based regressions, and compact/large viewport QA pass. |
