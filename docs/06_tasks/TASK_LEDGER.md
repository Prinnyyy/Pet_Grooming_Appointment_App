...
| T-004 | Establish Supabase profile foundation | planned | Deep | new Supabase project, scaffold, profile migration, avatar Storage, RLS, backend contract | explicit backend validation | Paused; legacy project forbidden; new project and cost confirmation required; API key local/ignored |
| T-005 | Add iOS Supabase client and session boundary | planned | Deep | Xcode dependency, Core infrastructure, config docs | explicit iOS validation | Infrastructure only; no Auth screens |
| T-006 | Implement email/password authentication | planned | Deep | Auth repository, session state, Auth views | explicit Auth validation | No role profile creation |
| T-007 | Implement role onboarding and entry routing | planned | Deep | onboarding, profile creation, App routing | explicit RLS and routing validation | No detailed role profile features |
