...
The current customer `TabView` renders customer pet management on Home, request wizard/list/detail on Requests, and authenticated Account content. The current groomer `TabView` renders groomer profile/services/portfolio management on Account and generic placeholder content for Requests, Offers, Bookings, and Messages.
