...
- Page-level actions such as Back, Continue, Save, or Publish retain their keyboard-hidden page coordinate and may be covered by a docked keyboard. Use `.beckonStationaryPageAction` for this project-specific behavior and `DesignTokens.Layout.stationaryActionContentClearance` when scroll content must remain reachable behind that overlay. It offsets only for a bottom-docked keyboard; floating, split, hidden, and hardware keyboards produce zero action offset. A send/reply control whose sole purpose is active text entry is a true input accessory and may track the keyboard.
