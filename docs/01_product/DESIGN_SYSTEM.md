...
- Page-level actions such as Back, Continue, Save, or Publish retain their keyboard-hidden page coordinate. When a bottom-docked software keyboard appears, `.beckonStationaryPageAction` moves the complete measured action below the screen; dismissal returns it from the bottom. Use `DesignTokens.Layout.stationaryActionContentClearance` when scroll content must remain reachable behind that overlay. Floating, split, hidden, and hardware keyboards produce zero action offset. A send/reply control whose sole purpose is active text entry is a true input accessory and may track the keyboard.
...
Primary references: Apple HIG `https://developer.apple.com/design/human-interface-guidelines/virtual-keyboards` and `https://developer.apple.com/design/human-interface-guidelines/sheets`; SwiftUI keyboard safe area, `FocusState`, `scrollDismissesKeyboard`, `onScrollPhaseChange`, `presentationContentInteraction`, and `interactiveDismissDisabled`; UIKit `UIKeyboardLayoutGuide` and `UIScrollView.KeyboardDismissMode.interactive`.
