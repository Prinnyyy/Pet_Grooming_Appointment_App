-- Storage objects must be removed through the Storage API. The delete-account
-- Edge Function performs that cleanup after this transactional anonymization
-- succeeds and before the Auth user is soft-deleted.
...
comment on function app_private.request_account_deletion() is
  'Privileged helper for signed-in account deletion. Storage cleanup is performed through the Storage API before Auth soft deletion.';
