-- T-392 / V-04. Freeze only soft evidence during a bounded browse.
-- Live hard qualification, membership and admission are never cached.
alter table app_private.match_ranking_config add column privacy_revision uuid not null default gen_random_uuid();

create table app_private.match_browse_snapshots (
  id uuid primary key default gen_random_uuid(),
  viewer_id uuid not null references public.profiles(id) on delete cascade,
  viewer_role text not null check(viewer_role in ('customer','groomer')),
  scope text not null,
  ...
  soft_evidence jsonb not null check(jsonb_typeof(soft_evidence)='object' and octet_length(soft_evidence::text)<=262144),
  check(isfinite(captured_at) and isfinite(valid_until) and valid_until>captured_at
    and valid_until<=captured_at+interval '5 minutes')
);
alter table app_private.match_browse_snapshots enable row level security;
revoke all on app_private.match_browse_snapshots from public,anon,authenticated,service_role;
create index match_browse_snapshots_viewer_age_idx on app_private.match_browse_snapshots(viewer_id,captured_at desc,id);
create index match_browse_snapshots_expiry_idx on app_private.match_browse_snapshots(valid_until,id);

create function app_private.invalidate_private_browse_evidence()
returns trigger language plpgsql security definer set search_path = '' as $$
declare invalidates boolean;
begin
  if tg_op='INSERT' then invalidates:=new.anonymized_at is not null;
  elsif tg_op='DELETE' then invalidates:=old.anonymized_at is not null;
  else invalidates:=new.anonymized_at is distinct from old.anonymized_at;
  end if;
  if invalidates then
...
revoke all on function app_private.invalidate_private_browse_evidence() from public,anon,authenticated,service_role;
create trigger account_deletion_browse_privacy after insert or update or delete on public.account_deletion_requests
for each row execute function app_private.invalidate_private_browse_evidence();

create function app_private.ranked_marketplace_page_with_evidence(p_role text,p_request uuid,p_sort text,p_limit integer,p_cursor text,p_soft jsonb)
...
revoke all on function app_private.ranked_marketplace_page_with_evidence(text,uuid,text,integer,text,jsonb)
  from public,anon,authenticated,service_role;

create function app_private.ranked_marketplace_browse(p_role text,p_request uuid,p_sort text,p_limit integer,p_cursor text)
...
begin
  ...
  if result->>'next_cursor' is not null then
    if p_cursor is null then
      soft_evidence:=result->'_soft_evidence';
      if octet_length(soft_evidence::text)>262144 then return result-'_soft_evidence'; end if;
      -- Only short retention writes serialize per viewer, never the ranking calculation.
      perform pg_advisory_xact_lock(hashtextextended('match-browse:'||actor::text,392));
      delete from app_private.match_browse_snapshots where viewer_id=actor and valid_until<=statement_timestamp();
      delete from app_private.match_browse_snapshots where id in (
        select id from app_private.match_browse_snapshots where viewer_id=actor
      ...
      values(actor,p_role,browse_scope,p_sort,result->>'algorithm_version',(soft_evidence->>'privacy_revision')::uuid,
        (result->>'score_as_of')::timestamptz,(result->>'valid_until')::timestamptz,soft_evidence)
...
revoke all on function app_private.ranked_marketplace_browse(text,uuid,text,integer,text)
  from public,anon,authenticated,service_role;

create function app_private.get_ranked_matched_requests_v2(p_sort text,p_limit integer default 25,p_cursor text default null)
returns jsonb language sql volatile security definer set search_path = '' as $$
  select app_private.ranked_marketplace_browse('groomer',null,p_sort,p_limit,p_cursor);
$$;
create function public.get_ranked_matched_requests_v2(p_sort text,p_limit integer default 25,p_cursor text default null)
returns jsonb language sql volatile security invoker set search_path = '' as $$
  select app_private.get_ranked_matched_requests_v2(p_sort,p_limit,p_cursor);
$$;
create function app_private.get_ranked_customer_offers_v2(p_request_id uuid,p_sort text,p_limit integer default 25,p_cursor text default null)
returns jsonb language sql volatile security definer set search_path = '' as $$
  select app_private.ranked_marketplace_browse('customer',p_request_id,p_sort,p_limit,p_cursor);
$$;
create function public.get_ranked_customer_offers_v2(p_request_id uuid,p_sort text,p_limit integer default 25,p_cursor text default null)
returns jsonb language sql volatile security invoker set search_path = '' as $$
  select app_private.get_ranked_customer_offers_v2(p_request_id,p_sort,p_limit,p_cursor);
$$;
revoke all on function app_private.get_ranked_matched_requests_v2(text,integer,text),public.get_ranked_matched_requests_v2(text,integer,text),
  app_private.get_ranked_customer_offers_v2(uuid,text,integer,text),public.get_ranked_customer_offers_v2(uuid,text,integer,text)
  from public,anon,authenticated,service_role;
grant execute on function app_private.get_ranked_matched_requests_v2(text,integer,text),public.get_ranked_matched_requests_v2(text,integer,text),
  app_private.get_ranked_customer_offers_v2(uuid,text,integer,text),public.get_ranked_customer_offers_v2(uuid,text,integer,text) to authenticated;

create function app_private.prune_match_browse_snapshots()
returns integer language plpgsql security definer set search_path = '' as $$
declare removed integer;
begin
  with expired as (select id from app_private.match_browse_snapshots where valid_until<=statement_timestamp()
    order by valid_until,id limit 500 for update skip locked)
  delete from app_private.match_browse_snapshots where id in (select id from expired);
  get diagnostics removed=row_count;
  return removed;
end $$;
revoke all on function app_private.prune_match_browse_snapshots() from public,anon,authenticated,service_role;
select cron.schedule('beckon_prune_match_browse_snapshots','* * * * *','select app_private.prune_match_browse_snapshots();');
