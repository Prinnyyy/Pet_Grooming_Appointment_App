-- Conservative raw-time bounds eliminate irrelevant history before exact release checks.
-- Effective release is constrained to the corresponding scheduled/occupied interval.
CREATE OR REPLACE FUNCTION app_private.evaluate_context_eligibility_cached(p_context public.grooming_requests, p_groomer uuid, p_now timestamp with time zone, p_valid_zones text[], INOUT p_weekly_cache jsonb, OUT evaluation jsonb)
 RETURNS record
 LANGUAGE plpgsql
 STABLE
 SET search_path TO ''
  ...
  select coalesce(range_agg(slot),'{}'::tstzmultirange) into blocked from (
    select tstzrange(coalesce(b.occupied_start,b.scheduled_start),app_private.booking_resource_end(b.scheduled_start,b.scheduled_end,b.occupied_start,b.occupied_end,b.resource_release_at),'[)') slot
      from public.bookings b where b.groomer_id=p_groomer and b.status in ('confirmed','completed','unfulfilled')
        and coalesce(b.occupied_start,b.scheduled_start)<r.preferred_end+make_interval(mins=>after_minutes)
        and coalesce(b.occupied_end,b.scheduled_end)>earliest-make_interval(mins=>before_minutes)
    union all
        ...
        loop
          ...
          select max(app_private.booking_pet_end(b.scheduled_start,b.scheduled_end,b.pet_release_at)) into pet_busy_until from public.bookings b
            where b.pet_id=r.pet_id and b.status in ('confirmed','completed','unfulfilled')
              ...
              and app_private.booking_pet_end(b.scheduled_start,b.scheduled_end,b.pet_release_at)>allocation.service_start;
    ...
    source:=jsonb_build_object('profile',g.eligibility_revision,
      ...
      'bookings',(select jsonb_agg(jsonb_build_array(b.id,b.status,b.scheduled_start,b.scheduled_end,
        ...
        from public.bookings b where (b.groomer_id=g.user_id or b.pet_id=r.pet_id)
          ...
          and greatest(b.scheduled_end,coalesce(b.occupied_end,b.scheduled_end))>r.preferred_start-interval '2 days'
