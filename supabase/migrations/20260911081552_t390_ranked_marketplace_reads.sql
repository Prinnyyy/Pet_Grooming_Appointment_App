-- T-390 / MR-05. New reads remain in time fallback until MR-06 enables ranking.
create table app_private.match_ranking_config (
  singleton boolean primary key default true check(singleton),
  signing_key bytea not null default extensions.gen_random_bytes(32),
  algorithm_version text not null default 'matching-v1' check(algorithm_version='matching-v1'),
  enabled boolean not null default false
);
alter table app_private.match_ranking_config enable row level security;
revoke all on app_private.match_ranking_config from public,anon,authenticated,service_role;
insert into app_private.match_ranking_config(singleton) values(true);

create function app_private.match_cursor_encode(p_payload jsonb,p_key bytea)
returns text language sql immutable set search_path = '' as $$
  select encode(convert_to(p_payload::text,'UTF8'),'hex')||'.'||
    encode(extensions.hmac(convert_to(p_payload::text,'UTF8'),p_key,'sha256'),'hex');
$$;
revoke all on function app_private.match_cursor_encode(jsonb,bytea) from public,anon,authenticated,service_role;

create function app_private.match_cursor_decode(p_cursor text,p_key bytea)
returns jsonb language plpgsql immutable set search_path = '' as $$
declare bytes bytea; supplied bytea; expected bytea; difference integer:=0; i integer;
begin
  if length(p_cursor)>16384 or p_cursor !~ '^[0-9a-f]+[.][0-9a-f]{64}$' then
    raise exception using errcode='22023',message='invalid_cursor';
  end if;
  bytes:=decode(split_part(p_cursor,'.',1),'hex');
  supplied:=decode(split_part(p_cursor,'.',2),'hex');
  expected:=extensions.hmac(bytes,p_key,'sha256');
  for i in 0..31 loop difference:=difference | (get_byte(supplied,i) # get_byte(expected,i)); end loop;
  if difference<>0 then raise exception using errcode='22023',message='invalid_cursor'; end if;
  -- Decode only authenticated bytes. Malformed untrusted tokens never become query values.
  return convert_from(bytes,'UTF8')::jsonb;
exception when invalid_parameter_value or character_not_in_repertoire or invalid_text_representation then
  raise exception using errcode='22023',message='invalid_cursor';
end $$;
revoke all on function app_private.match_cursor_decode(text,bytea) from public,anon,authenticated,service_role;

create function app_private.ranked_marketplace_page(p_role text,p_request uuid,p_sort text,p_limit integer,p_cursor text)
...
declare actor uuid:=(select auth.uid()); config app_private.match_ranking_config%rowtype;
  ...
  scoring_failed boolean:=false; fact_deadline timestamptz;
begin
  ...
  if p_cursor is not null then
    ...
    if deadline<=clock then raise exception using errcode='40001',message='list_changed'; end if;
  end if;
  ...
  loop
    ...
    if config.enabled then
      begin
        score:=app_private.score_match_evidence(request_id,groomer_id,as_of,offer_id);
  ...
  if p_cursor is not null and (cursor_data->>'ranking_revision' is distinct from revision
    ...
    raise exception using errcode='40001',message='list_changed';
  end if;
...
revoke all on function app_private.ranked_marketplace_page(text,uuid,text,integer,text) from public,anon,authenticated,service_role;

create function app_private.get_ranked_matched_requests(p_sort text,p_limit integer default 25,p_cursor text default null)
returns jsonb language sql stable security definer set search_path = '' as $$
  select app_private.ranked_marketplace_page('groomer',null,p_sort,p_limit,p_cursor);
$$;
create function public.get_ranked_matched_requests(p_sort text,p_limit integer default 25,p_cursor text default null)
returns jsonb language sql stable security invoker set search_path = '' as $$
  select app_private.get_ranked_matched_requests(p_sort,p_limit,p_cursor);
$$;
create function app_private.get_ranked_customer_offers(p_request_id uuid,p_sort text,p_limit integer default 25,p_cursor text default null)
returns jsonb language sql stable security definer set search_path = '' as $$
  select app_private.ranked_marketplace_page('customer',p_request_id,p_sort,p_limit,p_cursor);
$$;
create function public.get_ranked_customer_offers(p_request_id uuid,p_sort text,p_limit integer default 25,p_cursor text default null)
returns jsonb language sql stable security invoker set search_path = '' as $$
  select app_private.get_ranked_customer_offers(p_request_id,p_sort,p_limit,p_cursor);
$$;
revoke all on function app_private.get_ranked_matched_requests(text,integer,text),public.get_ranked_matched_requests(text,integer,text),
  app_private.get_ranked_customer_offers(uuid,text,integer,text),public.get_ranked_customer_offers(uuid,text,integer,text)
  from public,anon,authenticated,service_role;
grant execute on function app_private.get_ranked_matched_requests(text,integer,text),public.get_ranked_matched_requests(text,integer,text),
  app_private.get_ranked_customer_offers(uuid,text,integer,text),public.get_ranked_customer_offers(uuid,text,integer,text) to authenticated;
