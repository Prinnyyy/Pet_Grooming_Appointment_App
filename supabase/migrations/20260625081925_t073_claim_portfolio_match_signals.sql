-- T-073 groomer claim and portfolio match signals.
    ...
    greatest(
      ...
      least(
        ...
        eligible_groomer.location_score +
          ...
          coalesce(claim_tag_fit.adjustment, 0)
      )
    )::numeric(5, 2),
    left(
      case
        when pet_fit.reason_text is null
          and claim_tag_fit.reason_text is null
        then
  ...
  left join lateral (
    select
      ...
      string_agg(
        ...
        ', '
        order by
          ranked_evidence.evidence_points desc,
...
comment on function public.create_grooming_request(
  ...
  'Creates a fixed-service customer grooming request and creates eligible availability-aware groomer matches with location, bounded evidence-backed pet-fit scoring, and low-confidence groomer claim/portfolio signals.';
