-- Avoid row/admission lock inversion; retry only after subtransaction locks release.
...
declare actor uuid:=(select auth.uid()); actor_role public.user_role; deletion uuid; requested timestamptz:=statement_timestamp();
  item record; attempt integer; groomer_ids uuid[]; previous_redaction text:=current_setting('app.account_redaction_actor',true);
begin
  ...
  select array_agg(groomer_id order by groomer_id) into groomer_ids from (
    ...
    union select r.groomer_id from public.reviews r where actor in (r.customer_id,r.groomer_id)
    union select actor where exists(select 1 from public.groomer_profiles where public.groomer_profiles.user_id=actor)
  ) groomers;
  for item in select unnest(groomer_ids) groomer_id loop
    perform pg_catalog.pg_advisory_xact_lock(pg_catalog.hashtextextended(item.groomer_id::text,71071));
  end loop;
  -- Direct source UPDATEs lock their row before their admission-lock trigger.
  perform 1 from public.groomer_profiles g where g.user_id=any(groomer_ids)
    order by g.user_id for no key update nowait;
  perform 1 from public.groomer_services s where s.groomer_id=actor order by s.id for no key update nowait;
  perform 1 from public.customer_profiles c where c.user_id=actor for no key update nowait;
  perform 1 from app_private.address_locations a where a.owner_id=actor order by a.id for no key update nowait;
  select p.role into actor_role from public.profiles p where p.id=actor for no key update nowait;
  ...
  exception when lock_not_available then
    if attempt=3 then raise exception using errcode='PT409',message='account_deletion_retry'; end if;
